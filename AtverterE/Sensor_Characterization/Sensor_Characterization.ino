...
void controlUpdate(void) {
  ...
  // VALUE1 = ((double)atverterE.getActualVH() * 0.91) - 62;   // Collect the actual low voltage value
  // VALUE2 = ((double)atverterE.getActualVL() * 0.92) + 332;   // Collect the actual low voltage value
  ...
  SUM2 = SUM2 - READINGS2[INDEX];
  VALUE2 = ((double)(-atverterE.getIL()) * 0.95) + 30;  // Collect the actual low voltage value
  READINGS2[INDEX] = VALUE2;                          // Add the newest reading to the window
  SUM2 = SUM2 + VALUE2;                               // Add the newest reading to the sum
  INDEX = (INDEX + 1) % WINDOW_SIZE;                  // Increment the index, and wrap to 0 if it exceeds the window size
  AVERAGED2 = SUM2 / WINDOW_SIZE;
